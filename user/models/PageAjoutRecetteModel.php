<?php
class PageAjoutRecetteModel extends DBconnection
{
    public function ajouterRecette(
        ...
        $this->insererRecettescrees($idUser, $idRecette);
        $this->insererEtapes(
            $numEtape,
            $descriptionEtape,
            $idRecette
    ...
    public function insererRecette(
        ...
        try {
            ...
            $stmt->execute([
                ...
                "descriptionRecette" => $descriptionRecette, "idFete" => $idFete, "etat" => 0
            ]);
    ...
    public function insererRecettescrees($idUser, $idRecette)
    {
        try {
            $dataBase = $this->connecterDB($this->DBname, $this->host, $this->user, $this->password);
            $qry = "INSERT INTO recettescrees (idUtilisateur,idRecette) 
        VALUES (:idUser,:idRecette)";
            $stmt = $dataBase->prepare($qry);
            $stmt->execute(["idUser" => $idUser, "idRecette" => $idRecette]);
            $this->disconnect($dataBase);
        } catch (Exception $e) {
            echo 'Exception -> ';
            var_dump($e->getMessage());
